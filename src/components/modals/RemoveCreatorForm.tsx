...
export const RemoveCreatorForm = (props: any) => {
  ...
  const handleSubmit = (event: any) => {
    ...
    if (event.currentTarget.checkValidity() === false) {
      ...
      const addCreator = async () => {
        ...
        const tx = await contract.removeCreator(
          props.houseId,
        ...
        if (tx && tx.hash) {
          setTxResult(tx.hash)
          await tx.wait()
          setSuccess(`${event.target[0].value} removed successfully!`)
        }
    ...
    <Container>
      ...
      { !pending &&
        <Form noValidate validated={validated} onSubmit={handleSubmit} onChange={handleChange}>
          ...
          { ethereum &&
            <Button type='submit' variant='danger' disabled={!validated}>
              Remove creator
            </Button>
          }
        </Form>
